...
export class WebSocketClient extends EventEmitter implements IWebSocketClient {
    ...
    constructor() {
        ...
        console.log("[WebSocketClient] WebSocketClient created")
    }
    ...
    connect(url: string, options?: WebSocket.ClientOptions): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ws.on("ping", (data) => {
                console.log("Ping 수신:", data.toString())
                this.ws!.pong(data) // 수신된 ping의 payload 그대로 응답
                console.log("Pong 응답 전송:", data.toString())
            })
            ...
            this.ws.on("close", () => {
                console.error("[WebSocketClient] 'close' event fired")
                this.emit("close")
            })
    ...
    close(): void {
        ...
        console.error("[WebSocketClient] Closing websocket")
        this.ws.close()
    }
