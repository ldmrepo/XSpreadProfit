...
export class WebSocketManager extends EventEmitter {
    ...
    constructor(
        ...
        super()
        console.log("🚀 ~ WebSocketManager ~ id:", id, config)
    ...
    public async connect(): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            console.log("🚀 ~ WebSocketManager ~ 연결 ~")
            ...
            try {
                ...
                this.client
                    ...
                    .then(() => {
                        console.log("🚀 ~ WebSocketManager ~ 연결 성공 ~")
                        clearTimeout(timeoutId)
                        resolve()
                    })
                    .catch((error) => {
                        console.log("🚀 ~ WebSocketManager ~ 연결 실패 ~")
                        clearTimeout(timeoutId)
                        this.handleError(error)
                        reject(error)
                    })
    ...
    private handleOpen(): void {
        console.log("[WebSocketManager] handleOpen() called")
        this.reconnectAttempts = 0
        console.log("[WebSocketManager] Updating state to CONNECTED")
        this.updateState(ConnectorState.CONNECTED, {
            reason: "WebSocket onopen",
        })
        console.log("[WebSocketManager] State updated to CONNECTED")
        // this.setupPing()
        this.emit("connected")
        console.log("[WebSocketManager] 'connected' event emitted")
    }
    ...
    private processMessage(message: unknown): void {
        message = JSON.parse(JSON.stringify(message!.toString()))
    ...
    private handleClose(): void {
        ...
        if (this.state === ConnectorState.RECONNECTING) {
            // 이미 재연결 중이면 무시
            console.log("🚀 ~ 이미 재연결 중이면 무시")
            return
        }

        if (this.config.reconnectOptions?.maxAttempts) {
            console.log("🚀 ~ 재시도 1:", this.config.reconnectOptions)
            this.updateState(ConnectorState.RECONNECTING)
            this.handleReconnect().catch((error) => this.handleError(error))
        } else {
            console.log(
                "🚀 ~ 재시도 종료: WebSocketManager ~ handleClose ~ this.config.reconnectOptions?.maxAttempts:",
                this.config.reconnectOptions?.maxAttempts
            )
